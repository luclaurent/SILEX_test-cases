...
classdef wrapperSILEX < handle
    properties
        ...
        caseDefine='';
        %files for running
        resultFile='results/cavity_acou3D_struc_3D_v3results.mat';
        pythonMakeMesh={};
        pythonCompute={'Main_acou3D_struc3D_v3_grad.py'};
        %MPI options
        mpiOptNbCores='-np';
        mpiOptHost='hostfile';
    ...
    methods
        ...
        function cmd=buildCmdRun(obj,paraVal)
            ...
            if (obj.nbProc>1||obj.forceMPI)&&~isnan(obj.nbProc)&&isempty(obj.mpiOptHost)
            ...
            if ~isempty(obj.mpiOptHost)
                cmd=[cmd obj.varOpenblas ';' obj.mpiExecute ' --hostfile ' obj.mpiOptHost];
                if obj.numCurrent==1
                    obj.printWindows([' >> Mpi hostfile:' obj.mpiOptHost '\n'])
                end
            end
            ...
            if ~any(isnan(paraVal))&&(modROM||~obj.daemon)
                ...
                if obj.numCurrent==1
                    obj.printWindows([' >> Parameters values :' sprintf('%g ',paraVal) '\n'])
                end
            end

            if ~any(isnan(obj.gradCompute))&&(modROM||~obj.daemon)
                if numel(paraVal)>1
                    txtTmp=sprintf('%i,',obj.gradCompute(1:end-1));
                else
                    txtTmp='';
                end
                txtTmp=[txtTmp sprintf('%i',obj.gradCompute(end))];
                cmdOptPython=[cmdOptPython ' -g ' txtTmp];                
                if obj.numCurrent==1
                    obj.printWindows([' >> Gradients components :' sprintf('%g ',obj.gradCompute) '\n'])
                end
            end
