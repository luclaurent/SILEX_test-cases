...
import sys, os
sys.path.append(os.path.join(os.path.dirname(__file__),'../../../librairies'))
...
mesh_file=os.path.join(os.path.dirname(__file__),'geom','cavity_tet4')
results_file=os.path.join(os.path.dirname(__file__),'results','cavity_tet4_with_impedance_air_flexible_structure')

flag_write_gmsh_results=1

freq_ini     = 30.0
freq_end     = 120.0
nb_freq_step_per_proc=150
...
# shell structure
material_Struc=[]
material_Struc.append(15000.0e6) # E Young
material_Struc.append(0.4) # nu
material_Struc.append(8.0e-3) # thickness
material_Struc.append(800.0) # rho


# impedance paroi : article Walid-JFD : CMAME 2008
d_imp_paroi = 885.0 # Pa.s/m
k_imp_paroi = 1.4e6 # Pa/m
...
if (flag_write_gmsh_results==1) and (rank==0):
    ...
    silex_lib_gmsh.WriteResults(results_file+'_impedance_surface_Mesh_surface3',fluid_nodes,fluid_elements_S3,2)
...
if (Flag_frf_analysis==1):
    ...
    for i in range(nb_freq_step_per_proc):
        ...
        K=scipy.sparse.bmat( [ [KFF[SolvedDofF,:][:,SolvedDofF]+Kimp[SolvedDofF,:][:,SolvedDofF],KAF[SolvedDofF,:][:,SolvedDofA],None],
