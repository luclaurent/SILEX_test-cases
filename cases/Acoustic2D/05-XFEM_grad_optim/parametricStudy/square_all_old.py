...
def RunPb(freqMin, freqMax, nbStep, nbProc, rank, comm, paraVal, caseDefine):
    ...
    if Flag_frf_analysis == 1:
        ...
        if (flag_write_gmsh_results == 1) and (rank == 0):
            ...
            pR = np.real(press_save)
            pI = np.imag(press_save)
            pA = np.absolute(press_save)
            dataW.append(
                {
                    "data": pR,
                    "type": "nodal",
            ...
            itG = 0
            for itP in NamePara:
                dpR = np.real(dpress_save[itG])
                dpI = np.imag(dpress_save[itG])
                dpA = np.absolute(dpress_save[itG])
                ...
                dataW.append(
                    {
                        ...
                        "type": "nodal",
                        "nbsteps": len(freqCompute),
                        "name": "pressure gradient " + itP + " (deriv. absolute)",
                    }
                )
                itG = itG + 1
            # write fields
