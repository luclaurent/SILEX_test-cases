...
def RunPb(
    ...
    mesh_file = "geom/xfem3_noarea_fin_dim_b_rotate"
    results_file_ini = "results_curve_rotate/xfem_3_"
    ...
    ValpStruct = positionStructX * 10000.0
    ...
    LevelSet = (
        np.sqrt(
            (fluid_nodes[:, 0] - x_pos_struc) ** 2
            + (fluid_nodes[:, 1] - y_pos_struc) ** 2
        )
        - radius_hcircle
    )
    ...
    LevelSet_gradient_X = -(fluid_nodes[:, 0] - x_pos_struc) / (
        LevelSet + radius_hcircle
    )
    LevelSet_gradient_Y = -(fluid_nodes[:, 1] - y_pos_struc) / (
        LevelSet + radius_hcircle
    )
    ...
    if Flag_frf_analysis == 1:
        ...
        for freq in freqCompute:
            ...
            sol = scipy.sparse.linalg.spsolve(
                scipy.sparse.csc_matrix(K - (omega**2) * M, dtype="complex"), F
            )
            ...
            Dsol_Dtheta_X = scipy.sparse.linalg.spsolve(
                scipy.sparse.csc_matrix(K - (omega**2) * M, dtype="complex"), tmp_X
            )
            Dsol_Dtheta_Y = scipy.sparse.linalg.spsolve(
                scipy.sparse.csc_matrix(K - (omega**2) * M, dtype="complex"), tmp_Y
            )
            Dsol_Dtheta_R = scipy.sparse.linalg.spsolve(
                scipy.sparse.csc_matrix(K - (omega**2) * M, dtype="complex"), tmp_R
            )
