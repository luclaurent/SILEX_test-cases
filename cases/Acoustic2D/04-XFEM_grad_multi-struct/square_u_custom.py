...
def RunPb(freqMin,freqMax,nbStep,nbProc,rank,comm,positionStructX,positionStructY,radiusStruct,angleStruct):
    ...
    dK_T=scipy.sparse.bmat( [[None,fluid_damping*dKFA_dtheta_T[SolvedDofF,:][:,SolvedDofA]],
                                     [fluid_damping*dKFA_dtheta_T[SolvedDofA,:][:,SolvedDofF],None]
                                    ] )

    dM_T=scipy.sparse.bmat( [[None,dMFA_dtheta_T[SolvedDofF,:][:,SolvedDofA]],
    ...
    if (Flag_frf_analysis==1):
        ...
        for freq in freqCompute:
            ...
            DCorrectedPressure_Dtheta[SolvedDofA,0]=DCorrectedPressure_Dtheta[SolvedDofA,0].T+np.array(Denrichment_Dtheta[SolvedDofA,0]*np.sign(LevelSet[SolvedDofA]).T)
            DCorrectedPressure_Dtheta[SolvedDofA,1]=DCorrectedPressure_Dtheta[SolvedDofA,1].T+np.array(Denrichment_Dtheta[SolvedDofA,1]*np.sign(LevelSet[SolvedDofA]).T)
            DCorrectedPressure_Dtheta[SolvedDofA,2]=DCorrectedPressure_Dtheta[SolvedDofA,2].T+np.array(Denrichment_Dtheta[SolvedDofA,2]*np.sign(LevelSet[SolvedDofA]).T)
            DCorrectedPressure_Dtheta[SolvedDofA,3]=DCorrectedPressure_Dtheta[SolvedDofA,3].T+np.array(Denrichment_Dtheta[SolvedDofA,3]*np.sign(LevelSet[SolvedDofA]).T)
            #####################
            #####################
