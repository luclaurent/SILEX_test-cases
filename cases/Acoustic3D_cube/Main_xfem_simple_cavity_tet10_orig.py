...
from SILEXlib import silex_lib_acou_tet10
from SILEXlib import silex_lib_xfem_acou_tet10
...
results_file='results_orig/simple_cavity_tet10'
...
fluid_nodes1    = silex_lib_gmsh.ReadGmshNodes(str(cwd/(mesh_file+'_fluid.msh')),3)
fluid_elements1,IdNodes1 = silex_lib_gmsh.ReadGmshElements(str(cwd/(mesh_file+'_fluid.msh')),11,1) # air, cavity 
fluid_elements5,IdNodes5 = silex_lib_gmsh.ReadGmshElements(str(cwd/(mesh_file+'_fluid.msh')),11,5) # air, ONLY controlled volume
...
if (flag_write_gmsh_results==1) and (rank==0):
    silex_lib_gmsh.WriteResults(str(cwd/(results_file+'_air_cavity_Mesh1')),fluid_nodes1,fluid_elements1,11)
...
IIf,JJf,Vffk,Vffm=silex_lib_acou_tet10.globalacousticmatrices(fluid_elements1,fluid_nodes1,celerity,rho)
...
if (flag_write_gmsh_results==1) and (rank==0):
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_LS_signed_distance')),fluid_nodes1,fluid_elements1,11,[[[LevelSet],'nodal',1,'Level set']])
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_LS_tangent_level_set')),fluid_nodes1,fluid_elements1,11,[[[LevelSetTangent],'nodal',1,'Tangent level set']])
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_LS_distance')),fluid_nodes1,fluid_elements1,11,[[[distance],'nodal',1,'Distance']])
...
if (flag_write_gmsh_results==1) and (rank==0):
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_LSenriched_elements')),fluid_nodes1,fluid_elements1[LSEnrichedElements],11)
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_enriched_elements')),fluid_nodes1,fluid_elements1[EnrichedElements],11)
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_edge_enriched_elements')),fluid_nodes1,fluid_elements1[EdgeEnrichedElements],11)
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_edge_enriched_elements_in_all_mesh')),fluid_nodes1,fluid_elements1[EdgeEnrichedElementsInAllMesh],11)
    silex_lib_gmsh.WriteResults2(str(cwd/(results_file+'_heaviside_enriched_elements')),fluid_nodes1,fluid_elements1[HeavisideEnrichedElements],11)
...
toc = time.process_time()
if rank==0:
    print ("time to compute enrichment: {}".format(toc-tic))
