...
# load classes
objFEM = silex_lib_fem.LinearAcousticsTET10()
...
mesh_file = "geom/cube_acou_tet10"
results_file = "results/cube_acou_tet10"
...
fluid_elements = mesh.getElements(tag=1)["TET10"]
fluid_elements5 = mesh.getElements(tag=5)["TET10"]
fluid_elements6 = mesh.getElements(tag=6)["TRI6"]
...
if rank == 0:
    msh2.mshWriter(
        ...
        fluid_nodes,
        {"type": "TET10", "connectivity": fluid_elements},
    )
    msh2.mshWriter(
    ...
    msh2.mshWriter(
        ...
        fluid_nodes,
        {"type": "TRI6", "connectivity": fluid_elements6},
    )
...
if Flag_frf_analysis == 1:
    ...
    for freq in np.linspace(freq_ini, freq_end, nb_freq_step):
        ...
        sol = pymumps.spsolve(
            scipy.sparse.csc_matrix(K - (omega**2) * M, dtype="d"), F, comm=mycomm
        )
        # sol = mumps.spsolve( scipy.sparse.csc_matrix(fluid_damping*K-(omega**2)*M,dtype='c16') , F , comm=mycomm )
