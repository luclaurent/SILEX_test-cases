...
results_file = "results/simple_cavity_tet4"
...
if (flag_write_gmsh_results == 1) and (rank == 0):
    ...
    msh2.mshWriter(
        ...
        tangent_nodes,
        {"type": "TRI3", "connectivity": tangent_mesh},
    )
...
EnrichedElements = acousticsXFEM.getSurfEnrichedElements(
    fluid_nodes1, fluid_elements1[LSEnrichedElements], struc_nodes, struc_elements
)
...
EdgeEnrichedElements = acousticsXFEM.getEdgeEnrichedElements(
    fluid_nodes1, fluid_elements1, struc_nodes, struc_boun
)

EdgeEnrichedElementsInAllMesh = acousticsXFEM.getEnrichedElements(
    fluidElements=fluid_elements1, levelset=LevelSetTangent
)
...
if (flag_write_gmsh_results == 1) and (rank == 0):
    ...
    msh2.mshWriter(
        ...
        fluid_nodes1,
        {"type": "TET4", "connectivity": fluid_elements1[EdgeEnrichedElements]},
    )
    msh2.mshWriter(
    ...
    msh2.mshWriter(
        ...
        fluid_nodes1,
        {"type": "TET4", "connectivity": fluid_elements1[HeavisideEnrichedElements]},
    )
...
IIaa, JJaa, IIaf, JJaf, Vaak, Vaam, Vafk, Vafm = acousticsXFEM.getMatrices(
    fluid_nodes1, fluid_elements1[EnrichedElements], LevelSet, [celerity, rho]
)

KAAheaviside = scipy.sparse.csc_matrix(
    (Vaak, (IIaa, JJaa)), shape=(fluid_ndof1, fluid_ndof1)
)
MAAheaviside = scipy.sparse.csc_matrix(
    (Vaam, (IIaa, JJaa)), shape=(fluid_ndof1, fluid_ndof1)
)
KAFheaviside = scipy.sparse.csc_matrix(
    (Vafk, (IIaf, JJaf)), shape=(fluid_ndof1, fluid_ndof1)
)
MAFheaviside = scipy.sparse.csc_matrix(
    (Vafm, (IIaf, JJaf)), shape=(fluid_ndof1, fluid_ndof1)
)
...
# NbEltCutEdgeMEsh,LSnewnodes,LStgtnewnodes= silex_lib_xfem_acou_tet4.cutedgeglobalmesh(fluid_nodes1,fluid_elements1[EdgeEnrichedElements],LevelSet,LevelSetTangent)#II,JJ,vkaa,vmaa,vkfa,vmfa = silex_lib_xfem_acou_tet4.computeedgeenrichment(fluid_nodes1,fluid_elements1[PartiallyPositiveLStgtElements],LevelSet,LevelSetTangent,celerity,rNodesCutEdgeMesh,NbNodesCutEdgeMesh,EltCutEdgeMEsh,ho)
...
NodesCutEdgeMesh, EltCutEdgeMesh, LSnewnodes, LStgtnewnodes = (
    acousticsXFEM.getCutEdgeGlobalMesh(
        fluid_nodes1, fluid_elements1[EdgeEnrichedElements], LevelSet, LevelSetTangent
    )
)

msh2.mshWriter(
    cwd / (results_file + "_CutEdgeMEsh_LS.msh"),
    NodesCutEdgeMesh,
    {"type": "TET4", "connectivity": EltCutEdgeMesh},
    fields=[
        {"data": LSnewnodes, "type": "nodal", "dim": 1, "name": "Level set"},
        {"data": LStgtnewnodes, "type": "nodal", "dim": 1, "name": "Tangent level set"},
    ],
)
...
# II,JJ,vkaa,vmaa,vkfa,vmfa = acousticsXFEM.computeedgeenrichment2(fluid_nodes1,fluid_elements1[EdgeEnrichedElements],LevelSet,LevelSetTangent,celerity,rho)
II, JJ, vkaa, vmaa, vkfa, vmfa = acousticsXFEM.computeEdgeEnrichmenet(
    fluid_nodes1,
    fluid_elements1[EdgeEnrichedElements],
    LevelSet,
    LevelSetTangent,
    [celerity, rho],
)
...
toc = time.process_time()
if rank == 0:
    logger.info("time to compute edge enrichment: {}".format(toc - tic))
...
if Flag_frf_analysis == 1:
    ...
    for freq in np.linspace(freq_ini, freq_end, nb_freq_step):
        ...
        sol = pymumps.spsolve(
            scipy.sparse.csc_matrix(K - (omega**2) * M, dtype="float"), F, comm=mycomm
        )
