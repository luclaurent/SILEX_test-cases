...
nb_mode_S=44
nb_mode_A=900
results_file='results/xfem_struc_CB3_'+str(nb_mode_F)+'F-'+str(nb_mode_S)+'S-'+str(nb_mode_A)+'A'
...
if rank==0:
    Psi_FA=np.zeros((len(SolvedDofF),len(SolvedDofA)))
...
if rank==0:
    print ("time to compute PSI_FA: {}".format(toc-tic))
...
##################################################################
# Compute SVD of Psi_FA
##################################################################
tic = time.process_time()

U_Fk,S_kk,V_Ak=scipy.sparse.linalg.svds(Psi_FA, k=nb_mode_A, which='LM')
V_Ak=V_Ak.T

VSkk_diag_kk = S_kk
IISkk = list(range(nb_mode_A))
JJSkk = list(range(nb_mode_A))
S_kk= scipy.sparse.csc_matrix( (VSkk_diag_kk,(IISkk,JJSkk)), shape=(nb_mode_A,nb_mode_A) )

PsiTilde_Fk=U_Fk*S_kk
Psi_FA=np.array(Psi_FA.todense())

tmp=scipy.dot(Psi_FA.T,Psi_FA)
PsiFAT_PsiFA_inv=scipy.linalg.inv(tmp)
PsiHat_Ak=scipy.dot(scipy.dot(PsiFAT_PsiFA_inv,Psi_FA.T),PsiTilde_Fk)

#tmp = scipy.dot(scipy.dot(scipy.dot(scipy.dot(scipy.dot(V_Ak,S_kk),U_Fk.T),U_Fk),S_kk),V_Ak.T)
#PsiFAT_PsiFA_inv=scipy.linalg.inv(tmp)
#PsiHat_Ak=scipy.dot(scipy.dot(PsiFAT_PsiFA_inv*V_Ak*S_kk*U_Fk.T*U_Fk*S_kk)
#stop

toc = time.process_time()

if rank==0:
    print ("time to compute PSI_Hat_Ak: {}".format(toc-tic))
...
K_pk_1 = scipy.dot(np.array(eigen_vectors_F.T*KFF[SolvedDofF,:][:,SolvedDofF]),PsiTilde_Fk)
K_pk_2 = scipy.dot(np.array(eigen_vectors_F.T*KAF[SolvedDofF,:][:,SolvedDofA]),PsiHat_Ak)
K_pk = K_pk_1+K_pk_2

K_kk_1 = scipy.dot(PsiTilde_Fk.T*KFF[SolvedDofF,:][:,SolvedDofF],PsiTilde_Fk)
K_kk_2 = scipy.dot(PsiTilde_Fk.T*KAF[SolvedDofF,:][:,SolvedDofA],PsiHat_Ak)
K_kk_3 = K_kk_2.T
K_kk_4 = scipy.dot(PsiHat_Ak.T*KAA[SolvedDofA,:][:,SolvedDofA],PsiHat_Ak)
...
M_pk_1 = scipy.dot(np.array(eigen_vectors_F.T*MFF[SolvedDofF,:][:,SolvedDofF]),PsiTilde_Fk)
M_pk_2 = scipy.dot(np.array(eigen_vectors_F.T*MAF[SolvedDofF,:][:,SolvedDofA]),PsiHat_Ak)
M_pk = M_pk_1+M_pk_2

M_kk_1 = scipy.dot(PsiTilde_Fk.T*MFF[SolvedDofF,:][:,SolvedDofF],PsiTilde_Fk)
M_kk_2 = scipy.dot(PsiTilde_Fk.T*MAF[SolvedDofF,:][:,SolvedDofA],PsiHat_Ak)
M_kk_3 = M_kk_2.T
M_kk_4 = scipy.dot(PsiHat_Ak.T*MAA[SolvedDofA,:][:,SolvedDofA],PsiHat_Ak)
...
staticT_KSS_static_11 = np.array(S*KSS_static_S1.todense())[0][0]

Knn = scipy.sparse.bmat( [[K_diag_nn,None],[None,staticT_KSS_static_11]] )

Mnn = scipy.sparse.bmat( [[M_diag_nn,None],[None,1.0]] )



K=scipy.sparse.bmat( [ [fluid_damping*K_diag_pp,K_pk,None],[K_pk.T,fluid_damping*K_kk,None],[None,-Cnk,Knn] ] )

M=scipy.sparse.bmat( [ [M_diag_pp,M_pk,None],[M_pk.T,M_kk,Cnk.T],[None,None,Mnn] ] )
...
if (Flag_frf_analysis==1):
    ...
    for i in range(nb_freq_step_per_proc):
        ...
        P_A        = PsiHat_Ak*alpha_k
        press      = eigen_vectors_F*alpha_p+PsiTilde_Fk*alpha_k
