...
results_file='results/classic_pgd_fluid_cavity_with_modes'
...
ndof_x=nb_mode_F
ndof_w=omega_ndof
SolvedDofs_x=list(range(nb_mode_F))
...
for omi in range(nb_node_w):
    ...
    for i in range(nb_fcts_PGD):
        press[SolvedDofF]=press[SolvedDofF]+scipy.dot(eigen_vectors_F,F_i[i][list(range(nb_mode_F))])*G_i[i][omi]*alpha_i[i]
