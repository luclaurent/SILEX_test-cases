...
for t in range(n):
    ...
    Fsave.append(scipy.linalg.norm(Fext))
...
##    sigma[list(range(nnodes)),0]=Fint[list(range(0,ndof,3))]
##    sigma[list(range(nnodes)),1]=Fint[list(range(1,ndof,3))]
##    sigma[list(range(nnodes)),2]=Fint[list(range(2,ndof,3))]
##    tmp = copy.copy(sigma)
...
silex_lib_gmsh.WriteResults2(ResultsFileName+'_traction',nodes,elements,5,field_to_write)
