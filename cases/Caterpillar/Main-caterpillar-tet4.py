...
#############################################################################
print("SILEX CODE - calcul d'une piece caterpillar avec des tet4")
#############################################################################
...
MeshFileName='piece_caterpillar-tet4'

# Output result file: define the name of the result file (*.msh)
ResultsFileName='Results_caterpillar_tet4'
...
silex_lib_gmsh.WriteResults(ResultsFileName+'Volum',nodes,elements,eltype)
...
# Boundary conditions
IdNodesFixed_x=scipy.hstack([IdnodeS4])
IdNodesFixed_y=scipy.hstack([IdnodeS3])
IdNodesFixed_z=IdnodeS2

R=silex_lib_extra_python.turn_dof3D(IdnodeS4,nodes,[0.0,0.0])

# compute external forces from pressure
# pressure force
press = (435000.0/2.0)/(130.0*100.0) #MPa
direction = [0.0,0.0,0.0]
# distributed force
#press = (435000.0/2.0)/(0.5*130.0*scipy.pi*100.0) #MPa
...
F1 = silex_lib_elt.forceonsurface(nodes,elementsS5,press,direction)

press = (435000.0/2.0)/(0.5*130.0*scipy.pi*100.0) #MPa
direction = [0.0,1.0,0.0]
F2 = silex_lib_elt.forceonsurface(nodes,elementsS5,press,direction)
...
K=scipy.sparse.csc_matrix(R.T*K*R)
