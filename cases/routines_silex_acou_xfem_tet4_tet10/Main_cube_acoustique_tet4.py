...
mesh_file='cube_acou_h_10/cube_acou_tet4'
...
if (Flag_frf_analysis==1):
    ...
    for freq in numpy.linspace(freq_ini,freq_end,nb_freq_step):
        ...
        F=numpy.array(omega**2*F0 , dtype='d')
        #F=numpy.array(omega**2*F0 , dtype='c16')
        #F[SolvedDofF]=-(KFF[SolvedDofF,:][:,1-1]-(omega**2)*MFF[SolvedDofF,:][:,1-1])*(numpy.zeros((len([1])))+1.0)


        # sol=scipy.sparse.linalg.spsolve( scipy.sparse.csc_matrix(K-(omega*omega)*M+omega*D*1j,dtype=complex) , numpy.array(F.todense() , dtype=complex) )
        ...
        #sol = mumps.spsolve( scipy.sparse.csc_matrix(fluid_damping*K-(omega**2)*M,dtype='c16') , F , comm=mycomm )
        sol=scipy.sparse.linalg.spsolve( scipy.sparse.csc_matrix(fluid_damping*K-(omega*omega)*M,dtype=complex) , numpy.array(F , dtype=complex) )

        #press = numpy.zeros((fluid_ndof),dtype=float)
        press = numpy.zeros((fluid_ndof),dtype=complex)
