...
import sys
sys.path.append('/Users/luclaurent/Documents/1PRO/Recherche/Code/SILEXdev-nancite/librairies')
...
mesh_file='simple_cavity_h_10/simple_cavity_tet4'
...
# renumbering air
old = numpy.unique(fluid_elements1)
new = list(range(1,len(numpy.unique(fluid_elements1))+1))
new_nodes=fluid_nodes[numpy.unique(fluid_elements1)-1,:]

dico1 = dict(zip(old,new))
new_elements=numpy.zeros((fluid_nelem1,4),dtype=int)
...
EnrichedElements=numpy.unique(EnrichedElements[list(range(NbEnrichedElements))])
...
# #PartiallyPositiveLStgtElements=numpy.hstack([PositiveLStgtElements,EdgeEnrichedElementsInAllMesh])
...
# fluid node number 1
UF = numpy.zeros(2*fluid_ndof1,dtype=float)
UF[1-1]=3.1250E-05

SolvedDof = numpy.hstack([SolvedDofF,SolvedDofA+fluid_ndof1])
...
if (Flag_frf_analysis==1):
    ...
    for freq in numpy.linspace(freq_ini,freq_end,nb_freq_step):
        ...
        F=numpy.array(omega**2*UF[SolvedDof] , dtype='float')

        # sol = mumps.spsolve(  scipy.sparse.csc_matrix(K-(omega**2)*M,dtype='float')  , F , comm=mycomm  )
        sol=scipy.sparse.linalg.spsolve( scipy.sparse.csc_matrix(K-(omega**2)*M,dtype=float) , numpy.array(F , dtype=float) )

        press1 = numpy.zeros((fluid_ndof1),dtype=complex)
        press1[SolvedDofF]=sol[list(range(len(SolvedDofF)))]
        enrichment=numpy.zeros((fluid_nnodes),dtype=complex)
        enrichment[SolvedDofA]=sol[list(range(len(SolvedDofF),len(SolvedDofF)+len(SolvedDofA)))]
        CorrectedPressure=numpy.zeros((fluid_ndof1),dtype=complex)
        CorrectedPressure[SolvedDofA]=press1[SolvedDofA]+enrichment[SolvedDofA]*numpy.sign(LevelSet[SolvedDofA])
    ...
    frfsave=[numpy.array(frequencies),numpy.array(frf)]
