...
ResultsFileName='Results_assemblage_FRF_linear_beam3'
...
if flag_damping==1: # Fractionaire 
    mytype='c16'
...
Fprime = scipy.zeros(ndof+6*SuperNodes.shape[0],dtype=mytype)
...
Qprime=scipy.zeros(ndof+6*SuperNodes.shape[0],dtype=mytype)
...
Isuper,Jsuper,VKsuper = silex_lib_extra.superbeam3_stiff(elementBeam,'../diabolo/supelm.pkl')
...
if 1==0:
    K = scipy.sparse.csc_matrix(KSUPER+K3)
...
##        K = K1 + K2*Gstar/G0 + K3
    ...
    if flag_damping==1: # Maxwell / Fractionaire !!!
        # visco-elastic
        Gstar=(G0+Ginf*(1j*omega*tau)**alpha)/(1+(1j*omega*tau)**alpha)
        K = KSUPER*Gstar/G0 + K3 # attention, petite erreur: ici l'alu des diabolos aussi est amorti
        kk=scipy.sparse.csc_matrix(K[SolvedDofs,:][:,SolvedDofs]-(omega*omega)*M[SolvedDofs,:][:,SolvedDofs],dtype=mytype)

    Qprime[SolvedDofs] = mumps.spsolve( kk , scipy.array(Fprime[SolvedDofs],dtype=mytype)-(K[SolvedDofs,:][:,Fixed_Dofs]-(omega*omega)*M[SolvedDofs,:][:,Fixed_Dofs])*Qprime[Fixed_Dofs], comm=mycomm).T
    Q=R*Qprime
