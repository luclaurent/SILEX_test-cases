...
freq_ini     = 10.0
freq_end     = 1000.0
nb_freq_step_per_proc=700 # 50 pour 8 proc.
...
for x_pos_struc_ini in [5050]:#[6199,6200,6201]:
    h_struc=1.0
    ...
    K=scipy.sparse.construct.bmat( [[fluid_damping*KFF[SolvedDofF,:][:,SolvedDofF],KAF[SolvedDofF,:][:,SolvedDofA]],
    ...
    if (Flag_frf_analysis==1):
        ...
        for i in range(nb_freq_step_per_proc):
            ...
            Dsol_Dtheta = scipy.sparse.linalg.spsolve(  scipy.sparse.csc_matrix(K-(omega**2)*M,dtype='cfloat')  , tmp )
            #stop
...
pl.figure(1)
pl.plot(Allfrfsave[0],Allfrfsave[1],'ok-')
pl.plot(Allfrfsave[0],Allfrfsave[2],'og-')
...
pl.figure(2)
pl.plot(Allfrfsave[0],10*log10(Allfrfsave[1]/prefsquare),'k-')
pl.plot(Allfrfsave[0],Allfrfsave[2]/Allfrfsave[1]/log(10),'g-')
pl.show()