...
def RunPb(freqMin, freqMax, nbStep, nbProc, rank, comm, paraVal,gradValRequire=[],saveResults=1):#, caseDefine):
    ...
    results_file_ini = 'results/cavity_acou3D_struc_3D_v3_air_reduction'
...
##            Psi_IA[:,j]=scipy.array(Xi)[:,0]
    ...
    Freduced_F=(eigen_vectors_I.T).dot(UFreduc)
    ...
    if (Flag_frf_analysis == 1):
        ...
        for freq in freqCompute:
            ...
            P_A     = np.zeros((fluid_nnodes), dtype=complex)
            P_A[SolvedDofA] = sol_reduc[list(range(nb_mode_F,nb_mode_F+len(SolvedDofA),1))].copy()

            P_I = np.zeros((fluid_nnodes), dtype=complex)
            P_I[SolvedDofF]     = eigen_vectors_I.dot(alpha_m)+Psi_IA.dot(P_A[SolvedDofA])
            ## print('PI SHAPE',P_I.shape)
            



            #frf_reduc.append(silex_lib_xfem_acou_tet4.computecomplexquadratiquepressure(fluid_elements5,fluid_nodes,P_I))
            frf_reduc.append(silex_lib_xfem_acou_tet4.computexfemcomplexquadratiquepressure(
                fluid_elements5, fluid_nodes, P_I, P_A, LevelSet, LevelSet*0-1.0))
