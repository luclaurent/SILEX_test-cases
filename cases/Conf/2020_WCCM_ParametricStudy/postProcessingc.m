...
%file of the results
fileResult='xfem/parametricStudy/2017-05-05_18-47-45_porous_cavity';%
...
wX=linspace(0,120);
[wwX,wwY]=meshgrid(wX);
diffMax=zeros(size(wwX));
diffMin=zeros(size(wwX));
diffMean=zeros(size(wwX));
...
diffMax(itW)=max(FrFWMax(:))-min(FrFWMax(:));
diffMin(itW)=max(FrFWMin(:))-min(FrFWMin(:));
diffMean(itW)=max(FrFWMean(:))-min(FrFWMean(:));
end
fprintf('diff Max:%d\n',diffMax);
fprintf('diff Min:%d\n',diffMin);
fprintf('diff Mean:%d\n',diffMean);
