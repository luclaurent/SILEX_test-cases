...
def RunPb(freqMin, freqMax, nbStep, nbProc, rank, comm, paraVal,gradValRequire=[],saveResults=1):#, caseDefine):
    ...
    #LevelSet_from_Mesh,distance_from_Mesh = silex_lib_xfem_acou_tet4.computelevelset(fluid_nodes,struc_nodes,struc_elements)
...
def manageOpt(argv,dV):
    ...
    for opt,arg in opts:
        if opt == "-s":
            ...
            tmp = np.array(arg.split(','),dtype=scipy.int)
            gradCompute=tmp
        elif opt == "-h":
            usage()
            sys.exit()
