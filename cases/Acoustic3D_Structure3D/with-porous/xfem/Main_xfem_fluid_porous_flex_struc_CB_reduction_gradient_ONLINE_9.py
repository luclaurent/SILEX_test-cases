            ...
            for k in range(nproc):
                if k!=0:
                    if i+k<len(SolvedDofA):
                        ...
                        DPsi_IA_Dtheta[:,j]=np.array(DXi)[:,0]
                else:
                    DPsi_IA_Dtheta[:,j]=np.array(DXi)[:,0]
        i=i+nproc
...
if (Flag_frf_analysis==1):
    ...
    for i in range(nb_freq_step_per_proc):
        ...
        DK_Dtheta=scipy.sparse.bmat( [ [K_diag_mm*0.0,    None,               None,               None,                               None],
        ...
        DM_Dtheta=scipy.sparse.bmat( [ [None,             None,               DMhat_mA_Dtheta,    None,                               None],
        ...
        F  = scipy.append(omega**2*Freduced_F,np.zeros((len(SolvedDofA)+len(SolvedDofP)+nb_mode_S) , dtype='c16'))
        ...
        Dpress_Dtheta      = np.zeros(fluid_ndof1,dtype=complex)
        Dpress_Dtheta[SolvedDofI] = DP_I_Dtheta
        Dpress_Dtheta[SolvedDofB] = DP_B_Dtheta
        Denrichment_Dtheta = np.zeros(fluid_ndof1,dtype=complex)
    ...
    if rank==0:
        ...
        Allfrfsave=[np.array(list(Allfrequencies)),np.array(list(Allfrf)),np.array(list(Allfrfgradient))]
