###########################################################
## AIR CAVITY : CB reduction 
## POROUS MATERIAL : no reduction
...
# mpirun -np 20 python3 Main_xfem_CB_fluid_porous_7-v2.py
#
...
results_file='results/cavity7_CB_with_porous_air_v2'
...
if (Flag_frf_analysis==1):
    ...
    for i in range(nb_freq_step_per_proc):
        ...
        K=scipy.sparse.construct.bmat( [ [K_diag_mm,None,       None,   None],
                                         [None,     Khat_BB,    Khat_BA,None],
                                         [None,     Khat_BA.T,  Khat_AA,None],
                                         [None,     -CBP.T,     None,   KPP[SolvedDofP,:][:,SolvedDofP]]
                                         ]
                                       )
        
        M=scipy.sparse.construct.bmat( [ [M_diag_mm,    Mhat_mB,    Mhat_mA,    None],
                                         [Mhat_mB.T,    Mhat_BB,    Mhat_BA,    CBP],
                                         [Mhat_mA.T,    Mhat_BA.T,  Mhat_AA,    None],
                                         [None,         None,       None,       MPP[SolvedDofP,:][:,SolvedDofP]] ] )

        F  = scipy.append(omega**2*Freduced_F,scipy.zeros((len(SolvedDofA)+len(SolvedDofP)) , dtype='c16'))
