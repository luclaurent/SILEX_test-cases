...
Freduced_F=np.array(scipy.dot(PhiFm.T,UF[SolvedDofF]) , dtype='c16')
...
if (Flag_frf_analysis==1):
    ...
    for i in range(nb_freq_step_per_proc):
        ...
        K=scipy.sparse.bmat( [ [K_diag_mm,None,-CmP],
                                         [None,Khat_AA,-CAP],
                                         [None,None,KPP[SolvedDofP,:][:,SolvedDofP]] ] )

        M=scipy.sparse.bmat( [ [M_diag_mm,Mhat_mA,None],
...
##                                         [CmP.T,None,MPP[SolvedDofP,:][:,SolvedDofP]] ] )
        ...
        F  = scipy.append(F,np.zeros((len(SolvedDofA)+len(SolvedDofP)) , dtype='c16'))
