...
# mpirun -np 20 python3 Main_xfem_CB_fluid_porous_7-v3.py
#
...
results_file='results/cavity7_CB_with_porous_air_v3'
...
if (Flag_frf_analysis==1):
    ...
    for i in range(nb_freq_step_per_proc):
        ...
        K=scipy.sparse.construct.bmat( [ [K_diag_mm,None,       None,   None,                           None],
                                         ...
                                         [None,     None,       -CSA[SolvedDofS,:][:,SolvedDofA],   None,KSS[SolvedDofS,:][:,SolvedDofS]]
                                         ]
                                       )
        
        M=scipy.sparse.construct.bmat( [ [M_diag_mm,    Mhat_mB,    Mhat_mA,    None,                           None],
                                         [Mhat_mB.T,    Mhat_BB,    Mhat_BA,    CBP,                            None],
                                         [Mhat_mA.T,    Mhat_BA.T,  Mhat_AA,    None,                           CSA[SolvedDofS,:][:,SolvedDofA].T],
                                         [None,         None,       None,       MPP[SolvedDofP,:][:,SolvedDofP],None],
                                         [None,         None,       None,       None,                           MSS[SolvedDofS,:][:,SolvedDofS]]
                                         ]
                                       )

        F  = scipy.append(omega**2*Freduced_F,scipy.zeros((len(SolvedDofA)+len(SolvedDofP)+len(SolvedDofS)) , dtype='c16'))
        ...
        if rank==0:
            ...
            Q[SolvedDofS]=sol[list(range(nb_mode_F+len(SolvedDofB)+len(SolvedDofA)+len(SolvedDofP),nb_mode_F+len(SolvedDofB)+len(SolvedDofA)+len(SolvedDofP)+len(SolvedDofS),1))].real
    ...
    if rank==0:
        ...
        print('nb of total dofs: ',nb_mode_F+len(SolvedDofB)+len(SolvedDofA)+len(SolvedDofP)+len(SolvedDofS),)
