...
def RunPb(
    ...
    IIf, JJf, Vffk, Vffm = objFEM.getMatrices(fluid_nodes, fluid_elements1,[celerity, rho])
    ...
    IIaa, JJaa, IIaf, JJaf, Vaak, Vaam, Vafk, Vafm = objXFEM.getMatrices(fluid_nodes,
                                                                         fluid_elements1,
                                                                         LevelSet,                                                                         
                                                                         [celerity, rho],
                                                                         )
    ...
    if Flag_frf_analysis == 1:
        ...
        if (flag_write_gmsh_results == 1) and (rank == 0):
            ...
            dataW.append({'data':numpy.real(press_save), 'type':"nodal", 'nbsteps':itmax, 'name':"pressure (real)"})
            dataW.append({'data':numpy.imag(press_save), 'type':"nodal", 'nbsteps':itmax, 'name':"pressure (imaginary)"})
            dataW.append({'data':numpy.absolute(press_save), 'type':"nodal", 'nbsteps':itmax, 'name':"pressure (norm)"})
