...
import silex_lib_extra_fortran
#############################################################################
...
ResultsFileName='Results_truss_static_optim_1'
...
#############################################################################
    ...
    xe=silex_lib_extra.OptimalityCriteria(xeold,dC,Lelem,Lfrac,nelem)
...
# Build the new optimized mesh
new_elements=[]
for e in range(nelem):
    if xe[e]>0.95:
        new_elements.append(elements[e])
new_elements=scipy.array(new_elements)
nb_new_elements=new_elements.shape[0]

node_to_be_eliminated=scipy.setdiff1d(range(1,nnodes+1,1),scipy.unique(new_elements))
...
silex_lib_gmsh.WriteResults(ResultsFileName+'_xe_static_optim',new_nodes,new_elements,eltype,[])


#save xe:
f=open(ResultsFileName+'_xe_static_optim.pkl','wb')
pickle.dump([xe,dico],f)
f.close()
