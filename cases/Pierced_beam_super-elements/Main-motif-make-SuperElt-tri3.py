...
InterfaceIdNodes_1_left  = np.array(list(range(0,int(0.5*len(InterfaceIdNodes)),1)))
...
Ik = np.zeros(4*elements_super_3holes.shape[1]**2*nbelem_super_3holes,dtype=int)
Jk = np.zeros(4*elements_super_3holes.shape[1]**2*nbelem_super_3holes,dtype=int)
Vk = np.zeros(4*elements_super_3holes.shape[1]**2*nbelem_super_3holes,dtype=float)
...
Qall=np.zeros(nodes_3holes.shape[0]*2)

InternalDofs_1=InternalDofs
InternalDofs_2=InternalDofs+ndof
InternalDofs_3=InternalDofs+2*ndof

InterfaceDofs_1=np.array(InterfaceDofs)
InterfaceDofs_2=np.array(InterfaceDofs)+ndof
InterfaceDofs_3=np.array(InterfaceDofs)+2*ndof
...
# displacement written on 2 columns:
dispAll=np.zeros((nodes_3holes.shape[0],2))
...
loadAll=np.zeros((nodes_3holes.shape[0],2))
