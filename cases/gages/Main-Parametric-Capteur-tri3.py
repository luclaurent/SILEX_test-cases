...
cas=[]
A=scipy.linspace(-1.0,1.0,4)
B=scipy.linspace(-2.0,2.0,4)
E=[]
...
for a in A:
    ...
    for b in B:
        print("Cas : a=",a,"  ;  b=",b)
        make_capteur_mesh.WriteGmshGeoTri3(MeshFileName,[a,b,5.0,1.0])

        # read the mesh from gmsh
        nodes=silex_lib_gmsh.ReadGmshNodes(MeshFileName+'.msh',2)
        elements,Idnodes=silex_lib_gmsh.ReadGmshElements(MeshFileName+'.msh',2,1)
...
f=open(ResultsFileName+'_epsilon','wb')
pickle.dump([A,B,E], f)
f.close()
